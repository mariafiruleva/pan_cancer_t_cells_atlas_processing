...
parser$add_argument('--data',
                    type = "character",
                    help = 'Path to directory with 10x output counts & metadata file')
...
# whole <- get_object(args$data, args$annot)
...
# sapply(grep('snn_res', colnames(whole.integrated@meta.data), value = T),
...
unlink(args$sample_id, recursive = TRUE)
